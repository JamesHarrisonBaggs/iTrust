...
public class ICDCodeForm {
	...
	public ICDCodeForm(DAOFactory factory) {
		controller = new ICDCodeController(factory);
		search = "";
		setDisplayCodes(false);
	}

	public ICDCodeForm(ICDCodeController icdCodeController, DAOFactory factory) {
		if (icdCodeController == null) {
			controller = new ICDCodeController(factory);
		} else {
	...
	/**
	 * Logs a view action for each diagnosis code matching the current search
	 * query. Only logs if search query is non-empty.
	 */
