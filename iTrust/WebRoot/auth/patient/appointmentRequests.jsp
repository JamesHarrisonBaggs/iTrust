...
<%
	...
	List<PersonnelBean> hcpList = personnelDAO.getAllHCPPersonnel();
	String msg = "";
	...
	if (request.getParameter("request") != null) {
		...
		String hcpName = request.getParameter("HCP");
		String firstName = hcpName.substring(0, hcpName.indexOf(' '));
		String lastName = hcpName.substring(hcpName.indexOf(' ') + 1);
		List<PersonnelBean> hcpBeanList = personnelDAO.searchForPersonnelWithName(firstName, lastName);
		PersonnelBean hcpPerson = hcpBeanList.get(0);
		hcpid = hcpPerson.getMID();
...
<form action="appointmentRequests.jsp" method="post">
	...
	<select name="HCP">
		...
		%><option
			<%if (hcp.getFullName().equals(apptType))
					out.print("selected='selected'");%>
			value="<%=hcp.getFullName()%>"><%=hcp.getFullName()%></option>
		<%
			}
		%>
	</select>
