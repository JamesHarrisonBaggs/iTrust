...
<table>
	<tr>
			...
			<td align="center" colspan="2">
				<input name="NAME_SUBMIT" type="submit" value="User Search" />
			</td>
	</tr>
</table>
</form>
...
<div id="UserSearch">
	...
	<%
		...
		String url = StringEscapeUtils.escapeHtml("" + ( request.getParameter("forward") ));
	%>
			<input type="hidden" name="UID_PATIENTID" value="<%= StringEscapeUtils.escapeHtml("" + ( p.getMID() )) %>" />
				<tr <%=(index%2 == 1)?"class=\"alt\"":"" %>>
					<td>
						<input id="button" name="user_<%=StringEscapeUtils.escapeHtml("" + ( p.getMID() ))%>" type='submit' width='100%' value="<%=StringEscapeUtils.escapeHtml("" + p.getMID())%>" />
					</td>
					<td>
